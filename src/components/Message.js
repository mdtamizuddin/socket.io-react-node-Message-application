...
const Message = ({ show, setShow, socket, showSide, setShowSide, selected, setSelected, }) => {
    ...
    const [showEmo, setEmo] = useState(false)
    return (
        <div className='w-full h-full flex flex-col justify-between'>
            ...
            <div className="sendBox flex items-center py-1 px-5 border-t mb-2 shadow mx-2 relative">
                {
                    showGif &&
                    <div className="gif shadow-lg">
                        <GifCard
                ...
                <div className='flex items-center'>
                    ...
                    <button
                        ...
                        className=' p-1 rounded-full primary ml-1'>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                    ...
                    <button
                        onClick={() => {
                            showEmo ? setEmo(false) : setEmo(true)

                            setGif(false)
                        }}
                        className=' p-1 rounded-full text-xl primary ml-1'>
                        😅
                    </button>
                </div>
                {/* btns End Here  */}
...
const MessageBoxR = ({ message }) => {
    const date = moment(message.date).startOf('hour').fromNow();
    ...
    return (
        <>
            ...
            `}>
                ...
                {
                    message.emoji &&
                    <div className='rounded-lg mt-4 overflow-hiddenu'>
                        <img width={150} src={message.emoji} alt="" />
                    </div>
                }
                {
                    message.sticker &&
                    <div className='mt-4 flex items-center'>
                        <span className='text-4xl'>{message.sticker}</span>
                    </div>
                }
                {
                    isYou &&
